...
### Article Search API
You look the number of articles with "inequality" for example. You get the number of hits and a sample of 10 articles. 

Loading packages:

```{r}
library("httr") #We can access any API with this package. Send a request to get data and get it. 
library("jsonlite")
library("tidyverse")
```
...
```{r, eval = FALSE}
apikey <- "ATjyl1PdubGbKbV9HMejBCtde7HucxH9"
```
...
```{r}
#July 1885
#Spritnf. If we have a string and we want to repeatedly change things. It puts placeholders (%g) we supply 3 numbers and it substitues them into %g.
